...
void TrackKLT::perform_detection_monocular(const std::vector<cv::Mat> &img0pyr, std::vector<cv::KeyPoint> &pts0,
  ...
  //lin; filter close features, make sure unit area(min_px_dist^2) only has one key point
  //lin; TODO: why filter at this step, not filter this after keypoint detection ??? 
  Eigen::MatrixXi grid_2d = Eigen::MatrixXi::Zero((int)(img0pyr.at(0).rows / min_px_dist) + 15, 
                                                  (int)(img0pyr.at(0).cols / min_px_dist) + 15);
  auto it0 = pts0.begin();
  auto it2 = ids0.begin();
  while (it0 != pts0.end()) {
  ...
  // Grider_STC::perform_griding(img0pyr.at(0), pts0_ext, num_featsneeded, grid_x, grid_y, min_px_dist);
