...
double compute_error(Feature *feature, 
  ...
  for(size_t i=0; i<feature->timestamps.at(0).size(); i++) {
    ...
    z << hi1 / hi3, hi2 / hi3;
    Eigen::Matrix<double, 2, 1> res = feature->uvs_norm.at(0).at(i) - z;
...
bool single_triangulation(Feature *feature, 
  ...
  if (std::isnan(p_A_F.norm()) || p_A_F(2) < min_dist || 
      p_A_F(2) > max_dist || std::abs(condA) > max_cond_number) {
    return false;
  }
...
bool single_gaussnewton(Feature *feature, 
  ...
  if (feature->p_FinA(2) < min_dist || feature->p_FinA(2) > max_dist ||
      (feature->p_FinA.norm() / base_line_max) > max_baseline || 
